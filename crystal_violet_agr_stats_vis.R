#' ---
#' title: "agr subset of biofilm assays for publication, analyize and visualize the data"
#'output:
#'  html_document:
#'    toc: true
#'    theme: united
...
date_index <- norm_data[simple_id %in% c("agrA_", "agrA_pos1_agrA_20", "agrA_pos1_empty"),unique(date)] #find all experiments with the agr sample
norm_data <- norm_data[date %in% date_index][simple_id %in% c("wt_NA", "agrA_", "agrA_pos1_agrA_20", "agrA_pos1_empty")] #extract them from the data with thier matched wt
...
#+ filtered-plot
orddom_sliced <- slice(orddom_summary, c(1,4:6)) #filter
effsize_plot <- ggplot(orddom_sliced, aes(delta, comparison)) +
  ...
  scale_y_discrete(labels = c(l1, l2, l3, l7)) +
...
ggsave("Figures/crystal_violet_effsize.tiff", plot = effsize_plot, width = 30, height = 10, units = "cm", dpi = 1200) #this code is only evaluate when the script us run ourside of knitr
...
norm_data$simple_id <- factor(norm_data$simple_id, levels(norm_data$simple_id)[c(17,2,5,3)])
...
main_fig <- ggplot(norm_data, aes(simple_id, OD_adjusted)) +
  ...
  scale_x_discrete(labels = c(l1, l2, l7, l6)) +
  labs(x = " ", y = expression(normalized~OD[595])) +
  theme_mod + 
  geom_path(aes(x=rep(comparisons[[1]], each = 2),y=c(9,9.2,9.2,9.2)), data = facet1) + #modified to not cross the whisker
  geom_text(aes(x=median(comparisons[[1]]),y=9.4,label=sig_matrix1[[1,5]]), data = facet1, parse = TRUE) +
  geom_path(aes(x=rep(comparisons[[2]], each = 2),y=c(10,10.2,10.2,10)), data = facet1) +
  geom_text(aes(x=median(comparisons[[2]]),y=10.4,label=sig_matrix1[[2,5]]), data = facet1, parse = TRUE) +
  geom_path(aes(x=rep(comparisons[[3]], each = 2),y=c(11,11.2,11.2,11)), data = facet1) +
  geom_text(aes(x=median(comparisons[[3]]),y=11.4,label=sig_matrix1[[4,5]]), data = facet1, parse = TRUE) +
  geom_path(aes(x=rep(comparisons[[4]], each = 2),y=c(9,9.2,9.2,9)), data = facet1) +
  geom_text(aes(x=median(comparisons[[4]]),y=9.4,label=sig_matrix1[[6,5]]), data = facet1, parse = TRUE)
main_fig

#+ save-graph2, eval=FALSE
ggsave("Figures/crystal_violet_biofilm.tiff", plot = main_fig, width = 25, height = 25, units = "cm", dpi = 1200) #this code is only evaluate when the script us run ourside of knitr
